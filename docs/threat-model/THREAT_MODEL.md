...
deterministic analyst examples, bounded scenario replay, baseline regression and
benchmark inspection, hypothesis ledgers, and evidence export, with persistent
API writes denied. Production
...
telemetry ingestion, reset, arbitrary file-upload, or command-execution API. The
browser bundle verifier accepts a local file for on-device parsing and hashing;
it does not upload that file or extract logical paths.
...
| Malicious local bundle                | Bounded strict JSON/UTF-8 parser, exact format checks, duplicate-key/path rejection, no archive extraction or network upload                                                                                                                                                                                                         | Untrusted bundle metadata remains unverified text; the verifier establishes hash consistency only; real ingestion would require a separate design                                                                         |
...
| Secrets exposure                      | Local `.env` loads without overriding explicit process values or interpolating variables; public mode skips it; diagnostics exclude secret values and database URLs; Next.js passes public mode and capability metadata to clients without upstream configuration; tests disable workstation `.env` loading; no full prompts in logs | Machine/environment compromise remains possible; managed deployment secrets and rotation remain operator responsibilities; do not configure secrets with `NEXT_PUBLIC_*`                                                  |
...
## V2 investigation boundaries

The following controls are implemented in the linked modules. They extend the
same local/public boundary; none turns this demo into an authenticated service.

| Threat                                                          | Implemented defense                                                                                                                                                                                                                                                                                                                                                                     | Residual risk                                                                                                                                                                         |
| --------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Replay resource abuse or fabricated progression                 | [replay.py](../../apps/api/aegisgraph/replay.py) accepts known scenarios, caps 5,000 events / 80 playback events / 600 frames / 1 MiB, detects once, and correlates observed prefixes; browser playback has no server session                                                                                                                                                           | The full bounded projection is available to the browser; playback is a presentation, not an access-control boundary; public budgets are shared                                        |
| Cache poisoning or unbounded cache keys                         | [scenarios.py](../../apps/api/aegisgraph/scenarios.py), [replay.py](../../apps/api/aegisgraph/replay.py), [rule_api.py](../../apps/api/aegisgraph/rule_api.py), and [analyst_benchmark.py](../../apps/api/aegisgraph/analyst_benchmark.py) cache finite serialized repository-owned values and return fresh decoded objects; replay/regression/benchmark cold computation is serialized | Repository changes require review; process-local caches do not coordinate replicas or prove fixture truth                                                                             |
| Evaluation answer keys entering AI context                      | [scenario_ground_truth.py](../../apps/api/aegisgraph/scenario_ground_truth.py) holds authored labels separately; [hypothesis_api.py](../../apps/api/aegisgraph/hypothesis_api.py) passes only scoped evidence to deterministic scenario analysis                                                                                                                                        | The finite synthetic corpus cannot establish real-world accuracy; public benchmark expectations are intentionally inspectable, not confidential answer keys                           |
| Executable rule mutation                                        | [rule_specs.py](../../apps/api/aegisgraph/rule_specs.py) restricts fixed rule identities/kinds to declared integer parameters and bounds; [rule_workflow.py](../../apps/api/aegisgraph/rule_workflow.py) denies public changes                                                                                                                                                          | Local API access allows the demo analyst's actions; the fixture corpus cannot prove a proposed rule is safe for unmodeled traffic                                                     |
| Forged PASS result or stale concurrent approval                 | [rule_workflow.py](../../apps/api/aegisgraph/rule_workflow.py) independently recomputes full-corpus gates, checks prior-run ownership/content and corpus/ruleset digests, locks the singleton head, and compares its generation; BLOCK is rejected                                                                                                                                      | Human reasons are unauthenticated labels; a privileged database owner or code change can bypass application policy                                                                    |
| Cross-case hypothesis or finding references                     | [hypotheses.py](../../apps/api/aegisgraph/hypotheses.py) validates scope and exact evidence sets; [hypothesis_workflow.py](../../apps/api/aegisgraph/hypothesis_workflow.py) scopes histories and linked findings to the incident and binds reviews to its current context digest/version                                                                                               | Case isolation is data validation, not tenant authorization; matching recorded approvals do not prove identity or authentic authorization                                             |
| Human acceptance mistaken for evidence or stale report approval | Evidence-derived status stays separate from review status; incident locks serialize relevant changes, stale contexts invalidate review applicability, and [report generation](../../apps/api/aegisgraph/services.py) preserves hypothesis status/citations/gaps                                                                                                                         | A human can accept a weak hypothesis; acceptance does not establish truth; local actor labels are not authenticated                                                                   |
| Cross-case export or accidental local-work disclosure           | [evidence_export.py](../../apps/api/aegisgraph/evidence_export.py) uses a committed read-only snapshot, validates scoped references/audit objects, and omits owner, evidence notes, findings, notes, audit, and review state from public exports                                                                                                                                        | Public mode still requires a pristine synthetic database; this projection is not a general-purpose real-data anonymizer                                                               |
| Bundle path traversal, malformed encoding, or parser exhaustion | [evidence_bundle.py](../../apps/api/aegisgraph/evidence_bundle.py) and the [browser verifier](../../apps/web/src/lib/evidence-bundle.ts) bound input to 2 MiB, 100 files, 256 KiB/file, depth 32, and 100,000 nodes; reject BOM/invalid UTF-8, duplicate keys, nonfinite values, lone surrogates, unsafe paths and case collisions; never extract or open logical paths                 | Browser hashing requires Web Crypto; an invalid or over-capacity bundle is rejected, not repaired or partially accepted                                                               |
| Replacing evidence and the manifest together                    | Exact UTF-8 SHA-256 and byte-length checks identify content changes relative to the supplied manifest; all verification results disclose the unsigned-manifest limitation                                                                                                                                                                                                               | Replacing both file content and its hash can still return VALID; case/time/projection/application metadata are unauthenticated; no source authenticity or chain of custody is claimed |
| Capability spoofing or staggered rollout mismatch               | [server-runtime.ts](../../apps/web/src/lib/server-runtime.ts) checks the public runtime contract; [capabilities.ts](../../apps/web/src/lib/capabilities.ts) recognizes only known exact versions and hides unavailable features; backend method/service checks enforce read-only behavior independently                                                                                 | Capability flags describe availability, not identity or permission; a compromised backend remains trusted by the frontend                                                             |
| Expensive public GET requests bypassing analyst limits          | [public_security.py](../../apps/api/aegisgraph/public_security.py) assigns fixed scenario analyst GETs to the same analyst bucket/concurrency cap as case analysis; other new reads share existing read/global limits                                                                                                                                                                   | Budgets reset per process and offer no visitor fairness or distributed DDoS defense; hosting costs still exist without OpenAI calls                                                   |

Export consistency and source authenticity are separate concerns. The local CLI
and browser verifier hash exact supplied UTF-8 text without Unicode or newline
normalization. They do not interpret logical contents as HTML, execute them, or
authenticate the manifest. See [the bundle contract](../EVIDENCE_BUNDLES.md) and
...
The V2 suites exercise scenario-label separation, replay causality and bounds,
public cache isolation, typed mutations, forged/stale regression gates, scoped
hypothesis histories and linked findings, stale context reviews, exported audit
references, and strict bundle parsing/tampering. The deterministic benchmark
evaluates versioned application obligations with controlled provider drafts; it
does not measure a live model's attack-success rate. CI separately rehearses
[clean public PostgreSQL initialization](../../scripts/validate_public_database.py)
and [populated V1 upgrades](../../scripts/validate_populated_upgrade.py), including
preservation of old rows, repeat upgrades, and PostgreSQL append-only triggers.
